...
class RKdAngle(nn.Module):
    def forward(self, student, teacher):
        ...
        with torch.no_grad():
            ...
            # torch.nn.functional.normalize(input, p=2, dim=1, eps=1e-12, out=None)   本质上就是按照某个维度计算范数，(范数 https://zhuanlan.zhihu.com/p/67120415)
            # p表示计算p范数（等于2就是2范数），dim计算范数的维度（这里为1，一般就是通道数那个维度）   
            norm_td = F.normalize(td, p=2, dim=2)   # 求范数距离，即单位距离
            t_angle = torch.bmm(norm_td, norm_td.transpose(1, 2)).view(-1) # 求余弦
            for t in t_angle:
                print()
            # aa = ['a', 'b', 'c']
        ...
        # # a_file = open("angle.txt", "a+")   # w 是覆盖, a+是不覆盖,追加写
...
def pdist02(e, squared=False, eps=1e-12):
    ...
    prod = e @ e.t()
    res = (e_square.unsqueeze(1) + e_square.unsqueeze(0) - 2 * prod).clamp(min=eps)  #torch.clamp 将输入input张量每个元素的夹紧到区间 [min,max]，并返回结果到一个新张量。
